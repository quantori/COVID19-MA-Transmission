...
type
  TPat = record  {Пациент}
    Coord: TIntegerPoint2D;  {Координаты в м}
    ...
    AvrgRadDist: Integer;  {Среднее для показательного распределения по расстоянию до зараженного в m. По окружности распределение равномерное.}
  ...
  TSamplesArr = array of TSamples;   // Массив троек выборок на все периоды прогноза
  ...
  private
    ...
    procedure LoadSessionFile(const aSessionFileName: String);
...
const
  ...
  GeoLeftTop: TDoublePoint2D = (-73.260462, 42.745496);    {Координаты на поверхности (Long; Lat в градусах) левой верхней точки (Williemstown)}
  GeoRightBottom: TDoublePoint2D = (-70.008509, 41.535820);    {Координаты на поверхности (Long; Lat в градусах) правой нижней точки (Chatham)}
  MapLeftTop: TIntegerPoint2D = (96, 91);  {Координаты на карте (X, Y) левой верхней точки (Williemstown)}
  MapRightBottom: TIntegerPoint2D = (1047, 564);  {Координаты на карте (X, Y) правой нижней точки (Chatham)}
  EarthRad = 6371210;
  ...
  MaxPatCount = 800000;   // Максимальное количество симулированных пациентов
...
var
  ...
  MaxSimulTime, CurrMinInfTime, CurrSimulTime, MaxDetDuration: Integer;    // Максимальное время для симуляции, минимальное время инфицирования и момент симулирования
  ...
  DetPatCount, PatCount: Integer;  // Количество первоначально зарегистрированных и обработанны пауиентов в списке PatList
  ...
  HiperParam, CurrHiperParam: TDoublePoint2DArr;  // Параметры модели: 0 - Detection; 1 - Age; 2 - Distance; 3- Contact Usual;  4 - Contact Dence. (Mean, StdDev)
...
function YearDayCalend(aCurrTime: Integer): String;
  {Возвращает текст с датой, соответствующей моменту aCurrTime относительно начального дня модели (CurrDeyDay с aCurrTime = 0)м}
begin
...
begin
  ...
  aMapDist:= MapRightBottom[0] - aMapLeftTop[0];  // Горизонтальное расстояние по карте
...
function GetAvrgRadDistParam(const aHiperParam: TDoublePoint2DArr; aAge: Integer): Integer;
  {Возвращает случайное значения параметра AvrgRadDist (m) в зависимости от возраста.}
var aVal: Double;
begin
...
procedure SetIntPatParam(var aPatParam: Integer; var aStr: String; aDelim: String);
  {Устанавливает очередное знаение целого параметра aPatParam по текущей строке aStr}
var aDataStr: String;
    aInd: Integer;
begin
...
procedure SetSamplesArrPeriod(const aStatusArr: TStatusArr; aTimePeriod: TIntPoint2D; aPeriodInd, aIterInd: Integer; var aSamplesArr: TSamplesArr);
  {Заполнение данными троек выборок в aSamplesArr для итерации aIterInd по aStatusArr для данной итерации. aTimePeriod определяет интервал в aStatusArr.
...
function SamplesArray(const aForeсcastSchedule: TIntPoint2DArr; aIterCount: Integer; const aStatusBar: TStatusBar): TSamplesArr;
  {Заполнение данными четверок выборок в aSamplesArr. aTimePeriod определяет интервал в aStatusArr.}
...
begin
  ...
  if aWeekInd = 0 then
    ...
    Result[0]:= (aWeekInd - 1)*7 - aDetectDayInd + 4;
    Result[1]:= Result[0] + 6;
  end;
...
begin
  ...
  for i:= 0 to aWeekCount - 1 do begin
    Result[aForeDays + i]:= EpiWeek(aForeWeekInd + i + 1, DetectDayInd);
...
function DayYearInd(aDateStr, aDelimDate: String): Integer;
  {Возвращает индекс дня в году по строке даты aDateStr}
...
begin
  ...
  Forecast:= ForecastArr(ForecastDay, ForeWeekCount, DetectDayInd, aIterCount, aHistIntervCount, LevelList, MainStatusBar);
  SaveForecastArr(Forecast, ForecastFileName);
...
begin
  CopyChart(ForecastChart);
end;

procedure TMainForm.CopyToFileMenuClick(Sender: TObject);
begin
  CopyChartToFile(ForecastChart, SaveDialog);
end;
...
begin
  ...
  LoadSessionFile(COVIDSessionFileName);
  SetOptions;
  ...
  StopEvaluation:= False;
  LoadDetectECCounts(CurrentDirectory + '\Data\MA DetectCount.csv', DetectECCounts, DetectDays);
  Correct(DetectECCounts);
  DetDaysCount:= DetectDays.Count;
  SumDetectECCounts:= SumDetECCounts(DetectECCounts);
  LoadEpicenters(CurrentDirectory +  '\Data\MA Epicenters.csv', Epicenters);
...
procedure TMainForm.LoadSessionFile(const aSessionFileName: String);
